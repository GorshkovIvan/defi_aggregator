...
func OptimisePortfolio(database *Database) []OptimisedPortfolioRecord {
	...
	fcn := func(x_weights []float64) float64 {
		...
		*/
		...
		// 8 - Calculate blended return and risk
		...
		// 9 - Call the optimizer
		var p0 []float64 // sized same as number of tokens
		...
		fmt.Print(len(result.X))	
		result_norm := nrm_pool_wgts(result.X, pool_tkn0s, pool_tkn1s, pool_ratios, deployable_portfolio_array)		
		fmt.Print(result_norm)
				
		fmt.Println("OPTIMIZATION COMPLETE..RETURNING!!!..")

		// XXX - add - pack the weights into NewOptimisedPortfolio
